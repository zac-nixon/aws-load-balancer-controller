...
func BuildRoutingRuleTransforms(gwRoute RouteDescriptor, gwRule RulePrecedence) ([]elbv2model.Transform, error) {
	lrcTransforms := retrieveCustomTransform(gwRule)
	if len(lrcTransforms) > 0 {
		// TODO -- Hmm, how to combine results with the route?
		return lrcTransforms, nil
	}
	...
	case GRPCRouteKind:
		// GRPCRoutes do not support URL / Host header Rewrite
		return []elbv2model.Transform{}, nil
	default:
		return nil, errors.New(string("Unknown route kind " + gwRoute.GetRouteKind()))
	}
}

// retrieveCustomTransform translates transforms from their Kubernetes form to their ELB form.
// NOTE: Only used for transforms defined in the LBC specific configuration.
func retrieveCustomTransform(ruleWithPrecedence RulePrecedence) []elbv2model.Transform {
	hostHeaderTransforms := elbv2model.Transform{
		Type:                    elbv2model.TransformTypeHostHeaderRewrite,
		HostHeaderRewriteConfig: &elbv2model.RewriteConfigObject{},
	}
	pathTransforms := elbv2model.Transform{
	...
	if rule.GetListenerRuleConfig() != nil {
		customTransforms := rule.GetListenerRuleConfig().Spec.Transforms
		for _, transform := range customTransforms {
			if transform.MatchIndexes == nil {
				translateLRCTransform(transform, &hostHeaderTransforms, &pathTransforms)
			} else {
				for _, index := range *transform.MatchIndexes {
					if index == matchIndex {
						translateLRCTransform(transform, &hostHeaderTransforms, &pathTransforms)
					}
				}
	...
	var transformList []elbv2model.Transform
	if len(hostHeaderTransforms.HostHeaderRewriteConfig.Rewrites) > 0 {
		transformList = append(transformList, hostHeaderTransforms)
	}

	if len(pathTransforms.UrlRewriteConfig.Rewrites) > 0 {
		transformList = append(transformList, pathTransforms)
	}

	return transformList
}

func translateLRCTransform(gwTransform elbv2gw.Transforms, hostHeaderTransform, urlRewriteTransform *elbv2model.Transform) {
	if gwTransform.HostHeaderRewrites != nil {
		for _, hostHeaderRewrite := range *gwTransform.HostHeaderRewrites {
			hostHeaderTransform.HostHeaderRewriteConfig.Rewrites = append(hostHeaderTransform.HostHeaderRewriteConfig.Rewrites, elbv2model.RewriteConfig{
				Regex:   hostHeaderRewrite.Regex,
				Replace: hostHeaderRewrite.Replacement,
			})
		}
	}

	if gwTransform.PathRewrites != nil {
		for _, pathRewrites := range *gwTransform.PathRewrites {
			urlRewriteTransform.UrlRewriteConfig.Rewrites = append(urlRewriteTransform.UrlRewriteConfig.Rewrites, elbv2model.RewriteConfig{
				Regex:   pathRewrites.Regex,
				Replace: pathRewrites.Replacement,
			})
		}
	}
}

func buildHTTPRuleTransforms(rule *gwv1.HTTPRouteRule, httpMatch *gwv1.HTTPRouteMatch) ([]elbv2model.Transform, error) {
