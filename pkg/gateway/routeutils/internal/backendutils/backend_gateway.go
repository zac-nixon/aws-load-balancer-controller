...
func gatewayLoader(ctx context.Context, k8sClient client.Client, routeIdentifier types.NamespacedName, routeKind RouteKind, backendRef gwv1.BackendRef) (*GatewayBackendConfig, error, error) {
	if backendRef.Port == nil {
		...
		return nil, errors2.wrapError(errors.Errorf("%s", initialErrorMessage), gwv1.GatewayReasonListenersNotValid, gwv1.RouteReasonUnsupportedValue, &wrappedGatewayErrorMessage, nil), nil
	}
	...
	if gwIdentifier.Namespace != routeIdentifier.Namespace {
		...
		if !allowed {
			wrappedGatewayErrorMessage := routeutils.generateInvalidMessageWithRouteDetails(referenceGrantNotExists, routeKind, routeIdentifier)
			return nil, errors2.wrapError(errors.Errorf("%s", referenceGrantNotExists), gwv1.GatewayReasonListenersNotValid, gwv1.RouteReasonRefNotPermitted, &wrappedGatewayErrorMessage, nil), nil
		}
	}
	...
	if err != nil {
		...
		if convertToNotFoundError == nil {
			...
			wrappedGatewayErrorMessage := routeutils.generateInvalidMessageWithRouteDetails(initialErrorMessage, routeKind, routeIdentifier)
			return nil, errors2.wrapError(errors.Errorf("%s", initialErrorMessage), gwv1.GatewayReasonListenersNotValid, gwv1.RouteReasonBackendNotFound, &wrappedGatewayErrorMessage, nil), nil
		}
	...
	if arn == "" {
		...
		return nil, nil, errors2.wrapError(errors.Errorf("%s", initialErrorMessage), gwv1.GatewayReasonListenersNotValid, gwv1.RouteReasonBackendNotFound, &wrappedGatewayErrorMessage, nil)
	}

	err = validateGatewayARN(arn)
	if err != nil {
		wrappedGatewayErrorMessage := routeutils.generateInvalidMessageWithRouteDetails(err.Error(), routeKind, routeIdentifier)
		// This can be a warning, as we know that retrying reconcile will do nothing to fix this situation.
		return nil, errors2.wrapError(err, gwv1.GatewayReasonListenersNotValid, gwv1.RouteReasonBackendNotFound, &wrappedGatewayErrorMessage, nil), nil
	}
