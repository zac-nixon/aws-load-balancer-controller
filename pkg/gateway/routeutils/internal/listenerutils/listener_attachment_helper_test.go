...
func Test_listenerAllowsAttachment(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedStatusUpdate: &expectedRouteStatus{
				...
				message: routereconciler.RouteStatusInfoRejectedMessageNamespaceNotMatch,
			},
		},
		{
			name:             "kind is not ok",
			...
			expectedStatusUpdate: &expectedRouteStatus{
				reason:  string(gwv1.RouteReasonNotAllowedByListeners),
				message: routereconciler.RouteStatusInfoRejectedMessageKindNotMatch,
			},
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			route := &routeutils.httpRouteDescription{route: &gwv1.HTTPRoute{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "route1",
...
func Test_kindCheck(t *testing.T) {
	...
	testCases := []struct {
		name           string
		route          routeutils.preLoadRouteDescriptor
		listener       gwv1.Listener
		expectedResult bool
	}{
		{
			name:  "use fallback - https protocol, http route",
			route: &routeutils.httpRouteDescription{},
			listener: gwv1.Listener{
		...
		},
		{
			...
			route: &routeutils.tcpRouteDescription{},
			listener: gwv1.Listener{
		...
		},
		{
			...
			route: &routeutils.tlsRouteDescription{},
			listener: gwv1.Listener{
