...
func Test_convertListenerSetListenerToGatewayListener(t *testing.T) {
	...
	assert.Equal(t, routeutils.listenerParentKind(routeutils.listenerParentKindListenerSet), result.parentKind)
	assert.Equal(t, ls, result.parent)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			loader := &listenerSetLoaderImpl{
				namespaceSelector: &routeutils.mockNamespaceSelector{
					nss: tc.nsResult,
					err: tc.nsErr,
				},
				logger: logr.Discard(),
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectedListenerNames != nil {
				...
				for _, l := range listeners {
					names = append(names, l.listener.Name)
				}
